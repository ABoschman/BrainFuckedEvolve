...
impl<'a> BotInPlay<'a> {
    ...
    pub fn execute_code(&mut self, current_cell_is_zero: bool) -> Option<Mutation> {
        match self.bot.get_program()[self.code_pointer] {
            ...
            Instruction::Increment => Some(Mutation::new(self.pos as usize, self.polarity.mutation_relative_to_tape(1))),
            Instruction::Decrement => Some(Mutation::new(self.pos as usize, self.polarity.mutation_relative_to_tape(-1))),
